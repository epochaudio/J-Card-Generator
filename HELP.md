...
1.  **输入信息**:
    ...
    *   **文本导入**: 点击 "粘贴文本"，粘贴任意格式的曲目列表（如网易云/Spotify 歌单文本），AI 会自动识别。
    ...
    > **⚠️ 注意事项 (Note)**
    > *   **不支持单轨详细编辑**: 本工具专注于专辑整体视觉生成，UI 上不支持对单首曲目进行逐项修改。
...
*   **外观切换**: 支持 `明亮 (Light)` 和 `暗黑 (Dark)` 两种界面模式。
...
### 2.4 曲目管理 (Tracklist)
*   **智能导入 (Smart Import)**: 强烈推荐使用 `粘贴文本` 功能。支持从网易云、Spotify、MusicBrainz 或自行整理的文本中一键导入。
*   **AI 美化**: 点击顶部 `AI 风格与备注`，AI 会根据歌名自动补全“听感备注”和“心情描述”。
