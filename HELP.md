...
*   **翻转脊部 (Flip Spine)**: 在“布局选项”中新增了 **“翻转脊部”** 勾选框。
    *   **默认开启 (从上往下)**: 勾选时，脊部文字阅读方向为**从上往下 (Top-to-Bottom)**，符合大多数现代设计习惯。此模式下，编号位于顶部，艺术家位于底部。
    *   **传统模式 (从下往上)**: 取消勾选，即可一键切换回传统的**从下往上 (Bottom-to-Top)** 阅读方向，满足不同收藏爱好者的陈列习惯。
*   **组件重构**: 重写了脊部渲染引擎 (SpineContent)，底层架构更稳健，无论如何旋转，编号(ID)与艺术家(Artist)的物理位置都牢牢锚定在顶部和底部，不再乱跑。


### 版本 v1.3.1 更新说明 (Update Log)
#### 1. AI 导入引擎升级 (AI Import Engine)
*   **古典乐结构化支持**: 针对 "粘贴文本" 功能进行了 PRO 级升级。现在，系统能精准识别古典音乐的 "Work (作品)" 与 "Movement (乐章)" 层级。
*   **智能流派防护**: 引入了多流派兼容逻辑。遇到 Bill Evans 或 Pink Floyd 等标准专辑时，AI 会聪明地保持扁平歌单，不会错误地将被动词汇误判为作品组。

### 版本 v1.3.0 更新说明 (Update Log)
#### 1. AI 画风指令升级 (Prompt Engineering Refined)
*   **插画画风锁定**: 针对 "AI 生成提示词" 功能进行了深度优化。现在，系统强制规避"照片感"，并提供 4 种更精准的艺术插画风格：
    *   **A) 水彩/Gouache**: 流动的情绪与梦境感。
    *   **B) 90s Magazine**: 强构图、复古流行与留白。
    *   **C) Hand-drawn Diary**: 私密的手绘涂鸦与日记本质感。
    *   **D) Flat Design**: 几何原本与独立电子乐的极简平面感。
*   **具体落地**: 生成的指令将包含更具体的场景构建规则（如三轴呼应），确保 AI 出图的稳定性和艺术性。
