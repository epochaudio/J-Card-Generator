...
> 7. 🗂️ **Short Back 独立显示模式**: 新增 `AUTO / 档案信息 / 紧凑曲目` 三种折页模式。普通模式下也可单独切换左侧折页内容，并且不会影响主封底对拖拽后曲目顺序的真实渲染。
>
...
*   **背面智能密度控制 (Back Density Control)**:
    ...
    *   **非古典 Note-Aware 策略**: 在 `STANDARD / COMPILATION` 模式下，系统优先保留曲名与备注，再根据空间压力逐步隐藏 `artist / duration`。
    *   **真实字体测量驱动**: 所有降级决策基于实际字体折行与可用高度计算，而不是单纯按条目数粗暴裁剪。
*   **不仅仅是 J-Card**:
    *   支持 **4 折页布局 (U-Card)**，提供比传统 J-Card 更多的展示空间，完美包裹磁带盒。
    *   提供 **标准**、**古典**、**合辑** 三种排版模式。
    *   最左侧 `Short Back (Fold-in)` 现已支持独立模式选择，可在 `档案信息 / 紧凑曲目` 之间切换。
*   **专业打印适配**:
...
*   **Back Overflow Handling**:
    ...
    *   非古典模式会优先保留 `标题 + note`，再视密度隐藏 `artist / duration`。
